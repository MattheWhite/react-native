...
const GoalList = (props) => {
  // every function used by React which returns JSX will receive parameters/props automatically
  ...
  return (
    <ul className="goal-list">
      ...
      {props.goals.map((goal) => {
        return <li key={goal.id}>{goal.text}</li>;/* key prop can be tracked by React */
      })}
    </ul>
  );
};
